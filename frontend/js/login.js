...
try {
    ...
    var alertStructure = `
        <div class="alert alert-success d-flex align-items-center alert-container text-dark" role="alert">
            ...
            <div>Login Successful! Redirecting to Student Dashboard...</div>
        </div>
    `;
    ...
    // Store token in localStorage and redirect to Student Dashboard
    if (result.token) {
        localStorage.setItem("token", result.token);
        localStorage.setItem("admissionId", result.admission_id);
        setTimeout(() => {
            window.location.href = "/student-home"; // Redirect
        }, 2000);
    }
} catch (error) {
